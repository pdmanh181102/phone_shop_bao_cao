import { useUpdateUserLastName } from "@state/user/useUpdateUserLastName";
...
const UpdateLastNameDialog: React.FC<FormProps> = ({ user_uid, open, onCancel, onSuccess }) => {
    const [form] = Form.useForm<FormData>();

    const { updateName: updateUserLastName, loading } = useUpdateUserLastName(onSuccess);

    const handleSubmit = async () => {
        try {
            const values = await form.validateFields();
            updateUserLastName({ user_uid, last_name: values.name })
        } catch (error) {
...
export default UpdateLastNameDialog;