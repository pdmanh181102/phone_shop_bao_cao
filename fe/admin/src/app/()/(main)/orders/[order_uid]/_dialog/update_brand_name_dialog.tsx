...
import { useUpdateBrandName } from "@state/brand/useUpdateBrandName";
...
const UpdateBrandNameDialog: React.FC<FormProps> = ({ brand_uid, open, onCancel, onSuccess }) => {
    const [form] = Form.useForm<FormData>();

    const { updateName: updateBrandName, loading } = useUpdateBrandName(onSuccess);

    const { checkName: checkBrandName } = useCheckBrandName(async () => updateBrandName({ brand_uid, name: form.getFieldValue("name") }))
    ...
    return (
        <Modal title="Cập nhật tên" open={open} onCancel={onCancel} footer={null} width={500} destroyOnHidden>
            <Form form={form} layout="vertical" onFinish={handleSubmit} autoComplete="off">
                ...
                >
                    <Input placeholder="Nhập tên thương hiệu" maxLength={100} showCount />
                </Form.Item>
