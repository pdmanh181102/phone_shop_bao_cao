...
import { FetcherParams } from "../type/pagiation";
...
export class OrderApi {
  static async readAllItems(orderUid: string): Promise<OrderItem[]> {
    try {
      const response = await axiosMainServer.get<OrderItem[]>(
        `/${end_point}/${orderUid}/items`
      );
  ...
  static async readByUid(orderUid: string): Promise<Order> {
    try {
  ...
  static async create({
    ...
    try {
      const response = await axiosMainServer.post<Order>(`/${end_point}`, {
        note: note,
        address,
        recipientName,
        recipientPhone,
